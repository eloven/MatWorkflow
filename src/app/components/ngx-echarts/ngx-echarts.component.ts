import {
    Component, ElementRef, Input, Output, EventEmitter, ViewChild, AfterViewInit, OnDestroy, NgZone, OnChanges,
    SimpleChange, SimpleChanges, DoCheck, HostListener, OnInit
} from '@angular/core';
import { Subject, Subscription } from 'rxjs';
import { debounceTime } from 'rxjs/operators';

import { init, connect, disConnect } from 'echarts';
import { ECharts, EChartOption, EChartsConvertFinder, TypedArray } from 'echarts';

import { EChartInitOption, EChartSetOptionConfig, EChartsResizeOption } from './interface';
...
export class NgxEchartsComponent implements AfterViewInit, OnChanges, DoCheck, OnDestroy {
    ...
    init() {
        if (!this.echartsInstance) {
            ...
            if (!(this.initOptions && this.initOptions.height) && this.offsetHeight === 0) {
                this.el.nativeElement.style.height = '500px';
            }
            this.ngZone.runOutsideAngular(() => {
                this.onBeforeInit.emit();
                this.echartsInstance = init(this.host.nativeElement, this.theme, this.initOptions);
